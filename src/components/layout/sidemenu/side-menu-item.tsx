// src/components/layout/sidemenu/side-menu-item.tsx
"use client";
  ...
  return (
      <Tooltip>
        <TooltipTrigger asChild>
          <Link
              className={clsx(
                  "group relative block h-12 w-12 bg-foreground bg-cover transition-all hover:shadow-xl overflow-hidden",
          ...
          >
            ...
            {image && (
                <Image
                    ...
                    loader={({ src }) => `${src}`}
                    className={clsx(
                        "absolute inset-0 transition-all w-full h-full object-cover object-center scale-125", // Increased scale and added object-center
                        roundClasses
                    )}
                />
            )}
